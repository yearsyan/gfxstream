...
class Var {
   public:
    ...
    Var(const std::string& name, const VarType* vartype, const std::string& lenExpression,
        PointerDir dir, const std::string& packExpression, const std::string& unpackExpression,
    ...
    void init(const std::string name, const VarType* vartype, std::string lenExpression,
              PointerDir dir, std::string packExpression, std::string unpackExpression,
    ...
    const std::string& hostPackTmpAllocExpression() const {
        return (m_host_packTmpAllocExpression);
    }
    ...
    void setUnpackExpression(const std::string& unpackExpression) {
        m_unpackExpression = unpackExpression;
    }
    void setHostPackTmpAllocExpression(const std::string& expr) {
        m_host_packTmpAllocExpression = expr;
    }
    ...
    void setWriteExpression(const std::string& writeExpression) {
        m_writeExpression = writeExpression;
    }
    void setParamCheckExpression(const std::string& paramCheckExpression) {
        m_paramCheckExpression = paramCheckExpression;
    }
   ...
   private:
    ...
    std::string
        m_unpackExpression;  // an expression to unpack data that has arrived from the stream
    std::string m_host_packTmpAllocExpression;  // an expression to create temporaries for getting
                                                // into packed form for readbacks
    std::string
        m_host_packExpression;  // an expression to pack data into the stream but for readbacks
