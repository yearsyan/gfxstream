...
void ApiGen::printHeader(FILE* fp) const {
    fprintf(fp, "// Copyright 2025 The Android Open Source Project\n");
    fprintf(fp, "//\n");
    ...
    fprintf(fp, "// generated by './scripts/generate-apigen-sources.sh'\n");
}
...
int ApiGen::genProcTypes(const std::string& filename, SideType side) {
    ...
    for (size_t i = 0; i < size(); i++) {
        ...
        fprintf(fp, " (%s_APIENTRY *%s_%s_proc_t) (", basename, e->name().c_str(),
                sideString(side));
        if (side == CLIENT_SIDE) {
            fprintf(fp, "void * ctx");
        }
        ...
        if (side == SERVER_SIDE && e->customDecoder() && !e->notApi()) {
            fprintf(fp, "typedef ");
            e->retval().printType(fp);
            fprintf(fp, " (%s_APIENTRY *%s_dec_%s_proc_t) (", basename, e->name().c_str(),
                    sideString(side));
...
int ApiGen::genFuncTable(const std::string& filename, SideType side) {
    ...
    fprintf(fp, "};\n");
    fprintf(fp,
            "static const int %s_num_funcs = sizeof(%s_funcs_by_name) / sizeof(struct "
            "_%s_funcs_by_name);\n",
...
int ApiGen::genContext(const std::string& filename, SideType side) {
    ...
    StringVec& contextHeaders =
        side == CLIENT_SIDE ? m_clientContextHeaders : m_serverContextHeaders;
    ...
    // init function
    fprintf(fp,
            "\tint initDispatchByName( void *(*getProc)(const char *name, void *userData), void "
            "*userData);\n");
...
int ApiGen::genEntryPoints(const std::string& filename, SideType side) {
    ...
    fprintf(fp,
            "void %s_%s_context_t::setContextAccessor(CONTEXT_ACCESSOR_TYPE *f) { "
            "getCurrentContext = f; }\n",
            m_basename.c_str(), sideString(side));
    ...
    for (size_t i = 0; i < size(); i++) {
        ...
        fprintf(fp, "\t%sctx->%s(%s", shouldReturn ? "return " : "", e->name().c_str(),
                shouldCallWithContext ? "ctx" : "");
        size_t nvars = e->vars().size();

        for (size_t j = 0; j < nvars; j++) {
            if (!e->vars()[j].isVoid()) {
                fprintf(fp, "%s %s", j != 0 || shouldCallWithContext ? "," : "",
                        e->vars()[j].name().c_str());
            }
        }
        fprintf(fp, ");\n");
...
int ApiGen::genEncoderHeader(const std::string& filename) {
    ...
    fprintf(fp,
            "\t%s(gfxstream::guest::IOStream *stream, gfxstream::guest::ChecksumCalculator "
            "*checksumCalculator);\n",
            classname.c_str());
    ...
    if (var.isPointer()) {
        ...
        if (var.isDMA()) {
            ...
            if (dir == Var::POINTER_INOUT || dir == Var::POINTER_IN) {
                ...
                if (var.packExpression().size() != 0) {
                    ...
                    fprintf(fp, "memcpy(ptr, %s, __size_%s);", varname, varname);
                }
        ...
        if (!var.isVoid()) {
            fprintf(fp, "\t\tmemcpy(ptr, &%s, %u); ptr += %u;\n", varname,
                    (unsigned)var.type()->bytes(), (unsigned)var.type()->bytes());
        }
    }
}
...
static void writeEncodingChecksumValidatorOnReturn(const char* funcName, FILE* fp) {
    fprintf(
        ...
        "\t\t\tGFXSTREAM_FATAL(\"%s: GL communication error, please report this issue to "
        "b.android.com.\\n\");\n"
        "\t\t}\n"
        "\t}\n",
        funcName);
...
static void addGuestTimePrinting(const EntryPoint* e, bool hasTimeBeforeReadback, FILE* fp) {
#if INSTRUMENT_TIMING_GUEST
    fprintf(fp, "\tclock_gettime(CLOCK_REALTIME, &ts1);\n");
    fprintf(fp,
            "\tlong timeDiff = ts1.tv_sec*1000000 + ts1.tv_nsec/1000 - (ts0.tv_sec*1000000 + "
            "ts0.tv_nsec/1000);\n");
    if (hasTimeBeforeReadback) {
        fprintf(fp,
                "\tlong timeDiff2 = ts1.tv_sec*1000000 + ts1.tv_nsec/1000 - (ts2.tv_sec*1000000 + "
                "ts2.tv_nsec/1000);\n");
...
int ApiGen::genEncoderImpl(const std::string& filename) {
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        for (j = 0; j < maxvars; j++) {
            ...
            getVarEncodingSizeExpression(var, e, buff, sizeof(buff), EncodingSizeFlags::None);
            fprintf(fp, "%s;\n", buff);
        }
        ...
        for (size_t j = 0; j < nvars; j++) {
            npointers += getVarEncodingSizeExpression(
                ...
                (evars[j].isDMA() ? EncodingSizeFlags::DmaPtrOnly : EncodingSizeFlags::None) |
                    EncodingSizeFlags::UseExistingVar | EncodingSizeFlags::ExcludeOut);
            fprintf(fp, " + %s", buff);
        }
        ...
        if (hasLargeFields) {
            ...
            fprintf(fp,
                    "\tif (useChecksum) checksumCalculator->writeChecksum(ptr, checksumSize); ptr "
                    "+= checksumSize;\n\n");
        }
            ...
            if (evars[j].isPointer()) {
                ...
                if (dir == Var::POINTER_INOUT || dir == Var::POINTER_OUT) {
                    ...
                    if (evars[j].isDMA()) {
                        ...
                        fprintf(
                            fp,
                            "%sif (useChecksum) checksumCalculator->addBuffer(%s, __size_%s);\n",
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (e->unsupported()) {
            fprintf(fp, "\tthis->%s = (%s_%s_proc_t) &enc_unsupported;\n", e->name().c_str(),
                    e->name().c_str(), sideString(CLIENT_SIDE));
        } else {
            fprintf(fp, "\tthis->%s = &%s_enc;\n", e->name().c_str(), e->name().c_str());
        }
    }
    fprintf(fp, "}\n\n");
...
int ApiGen::genDecoderHeader(const std::string& filename) {
    ...
    fprintf(fp, "#include \"gfxstream/host/ChecksumCalculator.h\"\n");
    fprintf(fp, "#include \"gfxstream/host/iostream.h\"\n");
    ...
    fprintf(fp,
            "\tsize_t decode(void *buf, size_t bufsize, IOStream *stream, ChecksumCalculator* "
            "checksumCalc);\n");
    fprintf(fp, "\n};\n\n");
...
int ApiGen::genContextImpl(const std::string& filename, SideType side) {
    ...
    fprintf(fp,
            "int %s::initDispatchByName(void *(*getProc)(const char *, void *userData), void "
            "*userData)\n{\n",
            classname.c_str());
    for (size_t i = 0; i < n; i++) {
        EntryPoint* e = &at(i);
...
int ApiGen::genDecoderImpl(const std::string& filename) {
    ...
    fprintf(fp,
            "typedef unsigned int tsize_t; // Target \"size_t\", which is 32-bit for now. It may "
            "or may not be the same as host's size_t when emugen is compiled.\n\n");
    ...
    // decoder switch;
    fprintf(fp,
            "size_t %s::decode(void *buf, size_t len, IOStream *stream, ChecksumCalculator* "
            "checksumCalc) {\n",
            classname.c_str());
    fprintf(fp,
            "\tif (len < 8) return 0;\n\
...
\t\tif (end - ptr < packetLen) return ptr - (unsigned char*)buf;\n");
    ...
    for (size_t f = 0; f < n; f++) {
        ...
        for (int pass = PASS_FIRST; pass < PASS_LAST; pass++) {
            ...
            if (pass == PASS_FunctionCall) {
                ...
                if (strstr(m_basename.c_str(), "gl")) {
                    ...
                    fprintf(fp, "\t\t\tif (err) {\n");
                    fprintf(fp, "\t\t\t\tGFXSTREAM_ERROR(\"%s Error (pre-call): 0x%%X before ""%s\", err);\n", m_basename.c_str(), e->name().c_str());
                    fprintf(fp, "\t\t\t}\n");
                    fprintf(fp, "#endif  // ifdef CHECK_GL_ERRORS\n");
                }

                fprintf(fp, "\t\t\tDECODER_DEBUG_LOG(\"%s(%%p): %s(%s)\", stream",
            ...
            for (size_t j = 0; j < evars.size(); j++) {
                ...
                if ((pass == PASS_FunctionCall) && (j != 0 || e->customDecoder())) {
                    fprintf(fp, ", ");
                }
                ...
                if (v->isPointer() && v->isDMA()) {
                    if (pass == PASS_VariableDeclarations) {
                        fprintf(fp,
                                "\t\t\tuint64_t var_%s_guest_paddr = Unpack<uint64_t,uint64_t>(ptr "
                                "+ %s);\n"
                                "\t\t\t%s var_%s = stream->getDmaForReading(var_%s_guest_paddr);\n",
                                var_name, varoffset.c_str(), var_type_name, var_name, var_name);
                    }
                ...
                if (!v->isPointer()) {
                    if (pass == PASS_VariableDeclarations) {
                        fprintf(fp, "\t\t\t%s var_%s = Unpack<%s,uint%u_t>(ptr + %s);\n",
                                var_type_name, var_name, var_type_name, var_type_bytes * 8U,
                                varoffset.c_str());
                    }
                ...
                if (pass == PASS_VariableDeclarations) {
                    fprintf(fp,
                            "\t\t\tuint32_t size_%s __attribute__((unused)) = "
                            "Unpack<uint32_t,uint32_t>(ptr + %s);\n",
                            var_name, varoffset.c_str());
                }
                ...
                if (!v->isDMA()) {
                    if (v->pointerDir() & Var::POINTER_IN) {
                        if (pass == PASS_VariableDeclarations) {
                            ...
                            fprintf(fp, "\t\t\tInputBuffer inptr_%s(ptr + %s + 4, size_%s);\n",
                                    var_name, varoffset.c_str(), var_name);
                            if (v->unpackExpression().size() > 0) {
                                fprintf(fp,
                                        "\t\t\tvoid* inptr_%s_unpacked;\n"
                                        "\t\t\t%s;\n",
                                        var_name, v->unpackExpression().c_str());
                            }
                        }
                        if (pass == PASS_FunctionCall && v->pointerDir() == Var::POINTER_IN) {
                            if (v->nullAllowed()) {
                                fprintf(fp, "size_%s == 0 ? nullptr : (%s)(inptr_%s.get())",
                                        var_name, var_type_name, var_name);
                            } else {
                                if (v->unpackExpression().size() > 0) {
                                    fprintf(fp, "(%s)(inptr_%s_unpacked)", var_type_name, var_name);
                                } else {
                            ...
                            fprintf(fp, "unsigned char *inptr_%s = (ptr + %s + 4);\n", var_name,
                                    varoffset.c_str());
                        }
                        if (pass == PASS_FunctionCall && v->pointerDir() == Var::POINTER_IN) {
                            if (v->nullAllowed()) {
                                fprintf(fp, "size_%s == 0 ? NULL : (%s)(inptr_%s)", var_name,
                    ...
                    if (v->pointerDir() & Var::POINTER_OUT) {  // out pointer;
                        if (pass == PASS_TmpBuffAlloc) {
                            if (!totalTmpBuffExist) {
                                fprintf(fp, "\t\t\tsize_t totalTmpSize = size_%s;\n", var_name);
                            } else {
                                fprintf(fp, "\t\t\ttotalTmpSize += size_%s;\n", var_name);
                            }
...
#if USE_ALIGNED_BUFFERS
                            fprintf(fp, "\t\t\tOutputBuffer outptr_%s(&tmpBuf[%s], size_%s);\n",
                            ...
                            if (v->hostPackExpression() != "") {
                                fprintf(fp, "(%s)(forPacking_%s)", var_type_name, var_name);
                            } else {
                                if (v->nullAllowed()) {
                                    fprintf(fp, "size_%s == 0 ? nullptr : (%s)(outptr_%s.get())",
                                            var_name, var_type_name, var_name);
                                } else {
                        ...
                        if (pass == PASS_FlushOutput) {
                            if (v->hostPackExpression() != "") {
                                fprintf(fp,
                                        "\t\t\tif (size_%s) {\n"
                                        "\t\t\t%s; }\n",
                                        var_name, v->hostPackExpression().c_str());
                            }
                            fprintf(fp, "\t\t\toutptr_%s.flush();\n", var_name);
                        }
#else   // !USE_ALIGNED_BUFFERS
                            fprintf(fp, "\t\t\tunsigned char *outptr_%s = &tmpBuf[%s];\n", var_name,
                                    tmpBufOffset[j].c_str());
                            fprintf(fp, "\t\t\tmemset(outptr_%s, 0, %s);\n", var_name,
                                    toString(v->type()->bytes()).c_str());
                        } else if (pass == PASS_FunctionCall) {
                            if (v->nullAllowed()) {
                                fprintf(fp, "size_%s == 0 ? NULL : (%s)(outptr_%s)", var_name,
                                        var_type_name, var_name);
                            } else {
            ...
            if (pass == PASS_Protocol) {
                fprintf(fp,
                        ...
                        "\t\t\t}\n",
                        varoffset.c_str(), varoffset.c_str(), classname.c_str(), e->name().c_str());
            ...
            if (pass == PASS_FunctionCall || pass == PASS_DebugPrint) {
                ...
                if (pass == PASS_FunctionCall) {
                    if (!e->retval().isVoid() && !e->retval().isPointer()) {
                        fprintf(
                            fp,
                            "\t\t\tstd::memcpy(&tmpBuf[%s], &function_call_retval, sizeof(%s));\n",
                    ...
                    for (size_t j = 0; j < evars.size(); j++) {
                        ...
                        const char* var_name = v->name().c_str();
                        if (v->isDMA()) {
                            fprintf(fp, "\t\t\tstream->unlockDma(var_%s_guest_paddr);\n", var_name);
                        }
                    }
                }
            }
        ...
        fprintf(fp,
                ...
                "+ ts0.tv_nsec/1000);\n");
        fprintf(fp,
                "\t\t\tlong timeDiff2 = ts1.tv_sec*1000000 + ts1.tv_nsec/1000 - "
                "(ts2.tv_sec*1000000 + ts2.tv_nsec/1000);\n");
        fprintf(fp,
                "\t\t\tprintf(\"(timing) %%4ld.%%06ld %s: %%ld (%%ld) us\\n\", "
    ...
    if (strstr(m_basename.c_str(), "gl")) {
        ...
        fprintf(fp, "\t\tGLint err = this->glGetError();\n");
        fprintf(fp, "\t\tif (err) {\n");
        fprintf(fp, "\t\t\tGFXSTREAM_ERROR(stderr, \"%s Error (post-call): 0x%%X in %%s\", err, lastCall);\n",
                m_basename.c_str());
        fprintf(fp, "\t\t}\n");
        fprintf(fp, "#endif  // ifdef CHECK_GL_ERRORS\n");
    }
